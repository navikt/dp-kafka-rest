#!/bin/bash

export KAFKA_REST_ID=dp-kafka-rest
export KAFKA_REST_BOOTSTRAP_SERVERS=$KAFKA_BROKERS

# Schema Registry
export KAFKA_REST_SCHEMA_REGISTRY_URL=$KAFKA_SCHEMA_REGISTRY
export KAFKA_REST_SCHEMA_REGISTRY_USER=$KAFKA_SCHEMA_REGISTRY_USER
export KAFKA_REST_SCHEMA_REGISTRY_PASSWORD=$KAFKA_SCHEMA_REGISTRY_PASSWORD

# SSL/Security config for Kafka
export KAFKA_REST_SECURITY_PROTOCOL=SSL
export KAFKA_REST_SSL_KEYSTORE_TYPE_CONFIG=pkcs12
export KAFKA_REST_SSL_TRUSTSTORE_LOCATION=$KAFKA_TRUSTSTORE_PATH
export KAFKA_REST_SSL_TRUSTSTORE_PASSWORD=$KAFKA_CREDSTORE_PASSWORD
export KAFKA_REST_SSL_KEYSTORE_LOCATION=$KAFKA_KEYSTORE_PATH
export KAFKA_REST_SSL_KEYSTORE_PASSWORD=$KAFKA_CREDSTORE_PASSWORD

export KAFKA_REST_CLIENT_SECURITY_PROTOCOL=SSL
export KAFKA_REST_CLIENT_SSL_KEYSTORE_TYPE_CONFIG=pkcs12
export KAFKA_REST_CLIENT_SSL_TRUSTSTORE_LOCATION=$KAFKA_TRUSTSTORE_PATH
export KAFKA_REST_CLIENT_SSL_TRUSTSTORE_PASSWORD=$KAFKA_CREDSTORE_PASSWORD
export KAFKA_REST_CLIENT_SSL_KEYSTORE_LOCATION=$KAFKA_KEYSTORE_PATH
export KAFKA_REST_CLIENT_SSL_KEYSTORE_PASSWORD=$KAFKA_CREDSTORE_PASSWORD

exec /etc/confluent/docker/run